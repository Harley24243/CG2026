...
### <a href="https://mediasite.bris.ac.uk/Mediasite/Channel/d2f2d6b0f5184177b73cc931217fa1af5f" target="_blank"> Weekly Briefing ![](../../resources/icons/briefing.png) </a>
### Task 1: Introduction
...
In order to complete the work in this week's workbook, switch the `currentRenderer` in your main application to point at an instance of the `ColourSpectrumRenderer` class (note that `.cpp` and `.h` files have already been created for you). All code that you write for the tasks in this workbook should be entered into these `ColourSpectrumRenderer` files. This will help keep the code you write this week separate from the code you wrote last week (and also the code you will write in future weeks).
...
In your `RenderProject` code, add a new function called `interpolateSingleFloats` that takes 3 parameters:
...
For each pixel in the window, you will need to create a suitable colour by packing RGB channel data into a `uint32_t` (32 bit integer) variable. To find out more about pixels and pixel colour packing, take a look at the slides and audio narration for this section (the blue buttons above).  


![](03%20Single%20Dimension%20Greyscale%20Interpolation/images/grey-scale.jpg)

**Hints & Tips:**  
You might have to think a bit to figure out how to adapt the pixel looping code inside the `draw` function of `RedNoiseRenderer` in order to achieve the above gradient. Don't worry about the transparency/alpha channel of the pixels - just set it to 255 (fully opaque) as we did in the original code template.  
...
In order to use the `vec3` class, you will need to import the GLM header file with `#include <glm/glm.hpp>`. Note that you don't need to install the GLM library - this is already embedded within the `RenderProject` template (it's in the `libs` folder, if you are interested).
...
Note that the `vec2` and `vec3` objects from the GLM library support a range of primitive mathematical operators - so you can, for example, multiply a `vec2` by a float and get a scaled `vec2` as a result; or add two `vec2` objects together. This makes performing `vec2` operations a lot easier (since you don't need to perform operations on `x` then `y` components separately).  
