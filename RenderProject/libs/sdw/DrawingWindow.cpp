...
void DrawingWindow::saveBMP(const std::string &foldername, const std::string &filename) const {
	auto surface = SDL_CreateRGBSurfaceFrom((void *) pixelBuffer.data(), width, height, 32,
	                                        width * sizeof(uint32_t),
	                                        0xFF << 16, 0xFF << 8, 0xFF << 0, 0xFF << 24);
	std::string entirePath = foldername + separator + filename;
	SDL_SaveBMP(surface, entirePath.c_str());
}

void DrawingWindow::savePPM(const std::string &foldername, const std::string &filename) const {
	std::ofstream outputStream(foldername + separator + filename, std::ofstream::out);
	outputStream << "P6\n";
...
void DrawingWindow::toggleFullscreen() {
